//FILE: robot_with_light.cpp
//NAME: Jesse Johnson
//DATE: 12/5/2013
//CS 3014
//
//
// Displays a robot! Now with LIGHTS! The camera can be controlled to view the scene.
// It can move! Animates with keyboard commands. 
//
// 	
...
//________________________________________materials_and_lights___________________________
...
//define materials
	...
	//flat_red
	flat_red.ambient = color4( 0.7, 0.7, 0.7, 0.7 );
	flat_red.diffuse = color4( 1.0, 0.0, 0.0, 1.0 );
	flat_red.specular = color4( 0.0, 0.0, 0.0, 1.0 );
	flat_red.shininess = 0.0;

	//shiny_metal
	shiny_metal.ambient = color4( 1.0, 1.0, 1.0, 1.0 );
	shiny_metal.diffuse = color4( 0.7, 0.7, 0.7, 1.0 );
	shiny_metal.specular = color4( 1.0, 1.0, 1.0, 1.0 );
	shiny_metal.shininess = 50.0;

	//bright_bright_blue
	bright_blue.ambient = color4( 1.0, 1.0, 1.0, 1.0 );
	bright_blue.diffuse = color4( 0.0, 0.5, 1.0, 1.0 );
	bright_blue.specular = color4( 1.0, 1.0, 1.0, 1.0 );
	bright_blue.shininess = 100.0;

	//black_felt
	black_felt.ambient = color4( 0.0, 0.0, 0.0, 1.0 );
	black_felt.diffuse = color4( 0.0, 0.0, 0.0, 1.0 );
	black_felt.specular = color4( 0.4, 0.4, 0.4, 1.0 );
	black_felt.shininess = 5.0;

	//metallic_gold
	metallic_gold.ambient = color4( 1.0, 1.0, 1.0, 1.0 );
	metallic_gold.diffuse = color4( 0.255, 0.215, 0.0, 1.0 );
	metallic_gold.specular = color4( 1.0, 1.0, 1.0, 1.0 );
	metallic_gold.shininess = 70.0;

	//subdued_metal
	subdued_metal.ambient = color4( 0.5, 0.5, 0.5, 1.0 );
	subdued_metal.diffuse = color4( 0.5, 0.5, 0.5, 1.0 );
	subdued_metal.specular = color4( 0.0, 0.0, 0.0, 1.0 );
	subdued_metal.shininess = 20.0;

	//dark_soft
	dark_soft.ambient = color4( 1.0, 1.0, 1.0, 1.0 );
	dark_soft.diffuse = color4( 0.2, 0.2, 0.2, 1.0 );
	dark_soft.specular = color4( 0.2, 0.2, 0.2, 1.0 );
	dark_soft.shininess = 10.0;
...
//define lights
	...
	point_light_1.ambient =  color4( 0.01, 0.01, 0.01, 1.0 );
    	point_light_1.diffuse =  color4( 0.7, 0.7, 1.0, 1.0 );		
    	point_light_1.specular = color4( 0.5, 0.5, 0.5, 1.0 );
	point_light_1.position = vec4  ( -1.0, -1.0, 1.0, 1.0 );

	//point light 2
	point_light_2.ambient =  color4( 0.01, 0.01, 0.01, 1.0 );
	point_light_2.diffuse =  color4( 0.7, 1.0, 0.7, 1.0 );
	point_light_2.specular = color4( 0.5, 0.5, 0.5, 1.0 );
	point_light_2.position = vec4  ( 1.0,-1.0,-1.0, 1.0 );
	
	//directional light
	directional_light.ambient =  color4( 0.001, 0.001, 0.001, 1.0 );
	directional_light.diffuse =  color4( 0.4, 0.4, 0.5, 1.0 );
	directional_light.specular = color4( 1.0, 1.0, 1.0, 1.0 );
	directional_light.position = vec4  ( 1.0, 0.0, 1.0, 0.0 );
	...
	program = InitShader( "v_robot_with_light.glsl", "f_robot_with_light.glsl" );
...
void display( void )
{
	...
	glUniform4fv( glGetUniformLocation(program, "light_pos_1"),
		  1, polarView(zoomin, twist, elevation, azimuth) * point_light_1.position );

	glUniform4fv( glGetUniformLocation(program, "light_pos_2"),
		  1, polarView(zoomin, twist, elevation, azimuth) * point_light_2.position );

	glUniform4fv( glGetUniformLocation(program, "light_pos_dir"),
		  1, polarView(zoomin, twist, elevation, azimuth) * directional_light.position );
