...
// It walks around the scene in a circle. 
//
// 	
...
// 
// left and right arrows: lateral pan
// up and down arrows:    forward and backward
// a and z:		  up and down
// i and k:               vertical tilt
// j and l:               lateral turn
//
//
// light toggles
// 
// 1: left point light (white)
// 2: right point light (blue)
// d: directional light (white)
//	
...
vec4 cylinder_pt_normals[num_cylinder_vertices];
...
//disk variables
const int disk_slices = 100;
const int disk_rings = 100;
...
//call functions to plot points for primatives
	...
	Cylinder(base_radius, top_radius, height, cylinder_slices, cylinder_stacks, cylinder_points, cylinder_face_normals, 			cylinder_pt_normals);

	vec3 cylinder_normals[num_cylinder_vertices];

//fix issue regarding vec3/vec4 sizes
	for (int i = 0; i < num_cylinder_vertices; ++i)
	{
		cylinder_normals[i].x = cylinder_pt_normals[i].x;
		cylinder_normals[i].y = cylinder_pt_normals[i].y;
		cylinder_normals[i].z = cylinder_pt_normals[i].z;
...
//define materials
	...
	felt.specular = color4( 0.5, 0.5, 0.5, 1.0 );
	felt.shininess = 100.0;
...
//define lights
	...
	glBufferData( GL_ARRAY_BUFFER, 	sizeof(cube_points) +  
					sizeof(sphere_points) + 
					sizeof(cylinder_points) + 
					sizeof(disk_points) + 
					sizeof(cube_normals) + 
					sizeof(sphere_normals) + 
					sizeof(cylinder_normals)+ 
					sizeof(disk_normals),
					NULL, GL_STATIC_DRAW );
	...
	glBufferSubData( GL_ARRAY_BUFFER, sizeof(cube_points)+  sizeof(sphere_points)+ sizeof(cylinder_points)+sizeof(disk_points)+sizeof(cube_normals)+ sizeof(sphere_normals),sizeof(cylinder_normals), cylinder_normals);

	glBufferSubData( GL_ARRAY_BUFFER, sizeof(cube_points)+ sizeof(sphere_points)+ sizeof(cylinder_points)+sizeof(disk_points)+sizeof(cube_normals)+sizeof(sphere_normals)+sizeof(cylinder_normals), sizeof(disk_normals),disk_normals);




	// Load shaders and use the resulting shader program
	program = InitShader( "v_ec.glsl", "f_ec.glsl" );
	...
	glVertexAttribPointer( vNormal, 3, GL_FLOAT, GL_FALSE, 0,
			   BUFFER_OFFSET( sizeof(cube_points)+sizeof(sphere_points)+sizeof(cylinder_points)+
				sizeof(disk_points)));
...
void display( void )
{
	...
	//glUniformMatrix4fv( View_loc, 1, GL_TRUE, flyView(cam_x, cam_y, cam_z));
...

{
	...
	mvstack.push(model);
		...
		//model
		instance = Translate(0.0, -1.0, 0.0)*Scale(2.0, 0.05, 2.0);
		model *= instance ;	
		glUniformMatrix4fv( Model_loc, 1, GL_TRUE, model );
		//material
		applyMaterial(snow);
...
//scale and position of snowman  

	mvstack.push(model);
		
		//model
		instance = RotateY(t*(-0.01))*Translate(0.6, -0.8, 0.0)*Scale(0.3, 0.3, 0.3);
		instance *= Translate(0.6, -0.7, 0.0)*RotateZ(sin(t*(-0.01)))*Translate(-0.6, 0.7, 0.0);
	...
	mvstack.push(model);
		...
		glUniformMatrix4fv( Model_loc, 1, GL_TRUE, model );
		//material
		for (int i = 0; i < num_disk_vertices; i+=1000)
	
		applyMaterial(felt);
		glDrawArrays( GL_TRIANGLES, num_cube_vertices + num_sphere_vertices + num_cylinder_vertices, num_disk_vertices );
		model = mvstack.pop();
...
//camera controls
	...
	case '2':
		if (point_2 == 0){point_2 = 1;}
		else{point_2 = 0;}
		glUniform1i( glGetUniformLocation(program, "point_2"), point_2);
		glutPostRedisplay();
		break;
	...
	case 'd':
		...
		glUniform1i( glGetUniformLocation(program, "directional"), directional);
		glutPostRedisplay();
		break;
	...
	case 's':
		...
		glUniform1i( glGetUniformLocation(program, "spot"), spot);
		glutPostRedisplay();
		break;
